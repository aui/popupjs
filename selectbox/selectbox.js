/**
 * select 装饰器 @bingzitang
 * 用途：解决 select 无法自定义样式问题，功能仅此而已。
 * 请注意：相对原生的 select，模拟的下拉菜单会降低可用性以及丧失大量可编程接口。
 * 遇到需要自定义 select 的需求，请尽量搞定视觉设计师、拉拢交互设计师、并且与PM达成共识。
 * 最后如果大家仍然坚持装饰 select，请相信这是你见过的世界上最负责的控件，没有之一。
 * 已经模拟的部分：
 ...
 *		multiple 属性
 *		超出浏览器窗口弹出下拉框，包括 iframe
 * @param	{HTMLElement}	select 元素
 * @param	{Object}	选项。可用来指定装饰器的模板与样式名
 */
