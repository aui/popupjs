 ...
 * 浮层基础类
 * @author bingzitang
 * HTML5 Dialog Element API 支持
 * 元素吸附自适应定位与居中定位 | 智能焦点管理 | 全局 zIndex 自动管理 | 模态浮层支持 | UI 事件支持
 */
...
$.extend(Popup.prototype, {
	...
	show: function (anchor) {
		...
		this.__activeElement = _getActive();

		this.open = true;


		this.__popup.show().addClass('ui-popup-show');
		...
		if (!this.__ready) {
			...
			if (this.modal) {
				this.__popup.attr('role', 'alertdialog');
				this.__lock();
			}
	...
	focus: function () {
		...
		// 检查焦点是否在浮层里面
		if (!$.contains(node, _getActive())) {
			this.__focus(that.__popup.find('[autofocus]')[0] || node);
		}
	...
	blur: function () {
		...
		if (activeElement && !/^iframe$/i.test(activeElement.nodeName)) {
